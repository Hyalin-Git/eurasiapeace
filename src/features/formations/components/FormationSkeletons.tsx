export function FormationSkeleton({ single = true }: { single?: boolean }) {
  return (
...
export function FormationsSkeletons({ count = 6 }: { count?: number }) {
  ...
  return (
    <div className={`grid grid-cols-1 ${injectClassName()} gap-8`}>
      {Array.from({ length: count }).map((_, index) => (
        <FormationSkeleton key={index} single={false} />
      ))}
    </div>
  );
}
