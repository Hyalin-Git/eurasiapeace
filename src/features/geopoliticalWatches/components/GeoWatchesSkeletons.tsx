export function GeoWatchSkeleton({ single = true }: { single?: boolean }) {
  return (
...
export function GeoWatchesSkeletons({ count = 6 }: { count?: number }) {
  ...
  return (
    <div className={`grid grid-cols-1 ${injectClassName()} gap-8`}>
      {Array.from({ length: count }).map((_, index) => (
        <GeoWatchSkeleton key={index} single={false} />
      ))}
    </div>
  );
}
